...
export default function PublicForm() {
  ...
  useEffect(() => {
    async function init() {
      if (!slug) return;
      try {
        const res = await fetch(`http://localhost:8000/stage/form/${slug}`);
        if (res.ok) {
          const data = await res.json();
          setForm(data);
          
          // Log view event
          fetch(`http://localhost:8000/analytics/form/${data.id}/event`, {
            method: 'POST',
  ...
  const handleStart = () => {
    if (Object.keys(formData).length === 0 && form?.id) {
      fetch(`http://localhost:8000/analytics/form/${form.id}/event`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
