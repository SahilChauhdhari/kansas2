...
export default function PublicForm() {
  ...
  useEffect(() => {
    async function init() {
      if (!slug) return;
      try {
        const res = await fetch(`http://localhost:8001/stage/form/${slug}`);
        if (res.ok) {
          const data = await res.json();
          setForm(data);
          
          // Log view event
          fetch(`http://localhost:8001/analytics/form/${data.id}/event`, {
            method: 'POST',
  ...
  const handleStart = () => {
    if (Object.keys(formData).length === 0 && form?.id) {
      fetch(`http://localhost:8001/analytics/form/${form.id}/event`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch(`http://localhost:8001/stage/form/${slug}/submit`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
