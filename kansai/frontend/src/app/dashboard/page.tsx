...
export default function Dashboard() {
  ...
  const fetchForms = useCallback(async () => {
    if (!token) return;
    try {
      const res = await fetch('http://localhost:8000/workshop/forms', {
  ...
  const handleCreateForm = async () => {
    ...
    try {
      const res = await fetch('http://localhost:8000/workshop/forms', {
        ...
        body: JSON.stringify({ title: 'New FormFlow Project' })
      });
      if (res.ok) {
  ...
  return (
    <div className="dashboard-container">
            ...
            <div key={form.id} className="form-card">
              ...
              <div className="card-actions">
                ...
                <Link href={`/stage/${form.slug}`} target="_blank">
                  <button className="btn btn-small btn-secondary">Visit</button>
                </Link>
              </div>
            </div>
          ))}
      ...
      <style jsx>{`
        ...
        .card-actions { display: flex; gap: 0.75rem; margin-top: 2rem; }
