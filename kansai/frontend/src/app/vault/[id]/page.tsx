...
export default function VaultDashboard() {
  ...
  useEffect(() => {
    if (!id) return;
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/vault/form/${id}/responses`)
      .then(res => res.json())
      .then(data => {
         setResponses(data);
         setLoading(false);
       ...
       <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
             ...
             <div style={{ display: 'flex', gap: '1rem' }}>
                <a href={`${process.env.NEXT_PUBLIC_API_URL}/vault/form/${id}/export/csv`} target="_blank" rel="noreferrer">
                   <button className="btn btn-primary" style={{ background: 'var(--accent-4)', color: 'white' }}>
                      <i className="fas fa-file-csv"></i> Export CSV
                   </button>
                </a>
                <a href={`${process.env.NEXT_PUBLIC_API_URL}/vault/form/${id}/export/json`} target="_blank" rel="noreferrer">
