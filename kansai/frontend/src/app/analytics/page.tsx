...
export default function FormAnalytics() {
  ...
  useEffect(() => {
    const url = id ? `http://localhost:8000/analytics/form/${id}/metrics` : 'http://localhost:8000/analytics/global';
    fetch(url)
      .then(res => {
      ...
      .then(metrics => {
        ...
        const standardized = id ? {
            total_responses: metrics.completions,
            avg_completion_time: "2m 15s", // Calculation skipped for brevity
            conversion_rate: metrics.conversion_rate,
            timeline: [
                { name: 'Mon', responses: metrics.completions * 0.1 },
                { name: 'Tue', responses: metrics.completions * 0.2 },
                { name: 'Wed', responses: metrics.completions * 0.4 },
                { name: 'Thu', responses: metrics.completions * 0.3 },
                { name: 'Fri', responses: metrics.completions * 0.5 },
            ],
            devices: [
      ...
      <nav className="navbar">
        <div className="navbar-container">
          <div className="logo">
            ...
            <span className="logo-text">{id ? `FORM #${id}` : 'GLOBAL'} INSIGHTS</span>
          </div>
